    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      {tab === 'Zones' ? (
        <View style={{ flex: 1 }}>
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 24, paddingHorizontal: 12, paddingTop: 8 }}>
            <Text style={s.count}>{filtZones.length} zone{filtZones.length !== 1 ? 's' : ''}</Text>
            {filtZones.map((z) => (
              <View key={z.zone} style={zc.card}>
                {/* Card Header */}
                <View style={zc.cardHeader}>
                  <View style={zc.zoneNameRow}>
                    <Text style={zc.zoneName}>{z.zone}</Text>
                    <Text style={zc.hazard}>{z.mainHazard}</Text>
                  </View>
                  <Badge label={z.riskLabel} variant={z.riskLabel} />
                </View>

                {/* Score Bar */}
                <View style={zc.scoreRow}>
                  <Text style={zc.scoreLabel}>Risk Score</Text>
                  <Text style={[zc.scoreNum, { color: z.riskColor }]}>{z.score}</Text>
                </View>
                <Bar value={z.score} max={100} height={7} color={z.riskColor} />

                {/* Stats Row */}
                <View style={zc.statsRow}>
                  <View style={zc.stat}>
                    <Ionicons name="people-outline" size={14} color={C.t3} />
                    <Text style={zc.statVal}>{z.totalResidents}</Text>
                    <Text style={zc.statLbl}>Residents</Text>
                  </View>
                  <View style={zc.statDivider} />
                  <View style={zc.stat}>
                    <Ionicons name="heart-outline" size={14} color={C.purple} />
                    <Text style={zc.statVal}>{z.vulnerable}</Text>
                    <Text style={zc.statLbl}>Vulnerable</Text>
                  </View>
                  <View style={zc.statDivider} />
                  <View style={zc.stat}>
                    <Ionicons name="warning-outline" size={14} color={z.activeInc > 0 ? C.red : C.t3} />
                    <Text style={[zc.statVal, { color: z.activeInc > 0 ? C.red : C.t1 }]}>{z.activeInc}</Text>
                    <Text style={zc.statLbl}>Active Inc.</Text>
                  </View>
                </View>
              </View>
            ))}
...
const s = StyleSheet.create({
  ...
  count:     { fontSize: 11, color: C.t3, paddingHorizontal: 2, paddingVertical: 8 },
...
const zc = StyleSheet.create({
  ...
  zoneNameRow: { flex: 1, marginRight: 10 },
  zoneName:    { fontSize: 15, fontWeight: '800', color: C.t1 },
  hazard:      { fontSize: 11, color: C.t3, marginTop: 2 },
  scoreRow:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 },
  scoreLabel:  { fontSize: 10, color: C.t3, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.4 },
  scoreNum:    { fontSize: 13, fontWeight: '800' },
  statsRow:    { flexDirection: 'row', alignItems: 'center', marginTop: 12 },
  stat:        { flex: 1, alignItems: 'center', gap: 3 },
  statVal:     { fontSize: 14, fontWeight: '800', color: C.t1 },
  statLbl:     { fontSize: 9, color: C.t3, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.3 },
